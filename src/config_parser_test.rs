...
use crate::test_fixtures::{TEST_ENVINI_CONFIG_EXPAND_ENV_VAR, TEST_ENVINI_CONFIG_GOOD_1};
...
fn test_parsing_with_valid_file() {
    ...
    assert_that!(env_ini_mapping_entry.expand).is_equal_to(false);

    test_fixtures::cleanup(envini_config_ini);
}

#[test]
fn test_parsing_with_valid_file_with_expand() {
    // GIVEN is a test ini file with expand = true
    // AND arguments to the app
    let envini_config_ini = test_fixtures::get_ini_file(TEST_ENVINI_CONFIG_EXPAND_ENV_VAR);
    let args = vec![
        "".to_string(),
        envini_config_ini.0.to_str().unwrap().to_string(),
    ];
    ...
    assert_that!(env_ini_mapping_entry.is_some()).is_true();
    let env_ini_mapping_entry = env_ini_mapping_entry.unwrap();
    assert_that!(env_ini_mapping_entry.env_var_name).is_equal_to("KF2_WORKSHOP_ITEMS".to_string());
    assert_that!(env_ini_mapping_entry.ini_file).is_equal_to("test.ini".to_string());
    assert_that!(env_ini_mapping_entry.ini_section.as_ref().unwrap())
        .is_equal_to(&"OnlineSubsystemSteamworks.KFWorkshopSteamworks".to_string());
    assert_that!(env_ini_mapping_entry.ini_property_name)
        .is_equal_to("ServerSubscribedWorkshopItems".to_string());
    assert_that!(env_ini_mapping_entry.expand).is_equal_to(true);
